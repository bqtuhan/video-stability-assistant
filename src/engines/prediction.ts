...
const FREEZE_PROBABILITY_THRESHOLD = 0.35;
...
export function predictFreeze(
  ...
  const confidence: PredictionConfidence =
    history.length < 5 ? 'low' : history.length < 20 ? 'medium' : 'high';

  if (metrics.paused || metrics.readyState < 2) {
    return { willFreeze: false, probability: 0, estimatedSecondsUntilFreeze: null, confidence };
  }
  ...
  const probability =
    signals.bufferRunway * SIGNAL_WEIGHTS.bufferRunway +
  ...
  if (willFreeze && confidence !== 'low') {
    estimatedSecondsUntilFreeze = Math.round(metrics.bufferAhead);
  }
